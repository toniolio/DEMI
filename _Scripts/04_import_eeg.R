...
#### Prerequisite check: preprocessed EEG must exist ####
if (!file.exists("./_Data/eeg/prep_info.csv")) {
	stop("\nEEG PREPROCESSING REQUIRED\n",
		 "Missing _Data/eeg/prep_info.csv\n",
		 "Run the TraceLab EEG preprocessing pipeline first (separate repo),\n",
		 "then place outputs in:\n",
		 "  _Data/eeg/prep_info.csv\n",
		 "  _Data/eeg/edfs/sub-XXX_eeg_prepped.edf\n", call. = FALSE)
}
edf_files <- list.files("./_Data/eeg/edfs", pattern = "\\.edf$", full.names = TRUE)
if (length(edf_files) == 0L) {
	stop("\nEEG PREPROCESSING REQUIRED\n",
		 "No EDF files found in _Data/eeg/edfs\n",
		 "Expected filenames like sub-001_eeg_prepped.edf\n", call. = FALSE)
}
rm(edf_files)
...
if (file.exists(cached_eeg_path)) {
  ...
  eeg_data <- lapply(eeg_files, function(f) {
    ...
    sr <- eeguana::sampling_rate(eeg) # returns a single Hz value
    new_events <- update_events(eeg$.events, offsets_for_id, sr = sr)
    eeg$.events <- new_events

    # Separate EEG and EMG signals from full set of channels
    emg <- eeg %>% select(EMG_L, EMG_A)
    eeg <- eeg %>% select(-HEO, -VEO, -EMG_L, -EMG_A)
    ...
    if (id_num %in% imagery_ids) {
      epoched_emg <- emg %>%
        mutate(
          EMG_L = channel_dbl(emg_bandpass(EMG_L, 20, 450, srate = 1000)),
          EMG_A = channel_dbl(emg_bandpass(EMG_A, 20, 450, srate = 1000))
        ) %>%
        eeg_segment(
