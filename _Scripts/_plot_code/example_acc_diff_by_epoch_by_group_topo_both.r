...
(
	...
	# collapse accuracy to a difference score, dropping sample from the grouping thereafter
	%>% summarise(
...
(
	...
	%>% mutate(
		...
		, to_plot_x = x_scaled + epoch_scaled
...
(
	...
	+ geom_errorbar(
		mapping = aes(
			...
			, ymin = to_plot_lo
			, ymax = to_plot_hi
			, group = interaction(lat,long,group)
			, fill = group
		)
		, alpha = .5
...
#now save
ggsave(
	file = '_plots/example_epoch_by_group_topo_both.pdf'
	, width = 10
	, height = 10
)
